    ...
    <div className={`bg-${theme}`}>
      <div className="lg:flex lg:items-center lg:justify-between w-2/3 mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8 z-20">
        <h2 className="text-3xl font-extrabold text-black sm:text-4xl">
          {title && <span className="block font-jakarta">{title}</span>}
          {text && (
            <span className="block font-jakarta text-white">{text}</span>
          )}
        </h2>
                  ...
                  <a target={delve(button, 'link.target')}>
                    <button
                      ...
                      className={`py-4 px-6 bg-${delve(
                        ...
                        button,
                        'theme'
                      )}-text w-full font-jakarta transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2  rounded-lg`}
                    >
