      ...
      <div className="relative">
        <div className="lg:grid lg:grid-flow-row-dense lg:grid-cols-2 lg:gap-8 lg:items-center">
          <div className="lg:col-start-2 md:ml-24 lg:max-w-2xl">
            {label && (
              <p
                className={`font-black font-jakarta leading-6 text-${theme} uppercase`}
              >
                {label}
              </p>
            )}
            {title && (
              <h4 className="mt-2 text-3xl font-jakarta leading-8 font-black text-gray-900 sm:text-4xl sm:leading-9">
                {title}
              </h4>
            )}
            {text && (
              <p className="mt-4 font-jakarta text-lg leading-6 text-gray-500">
                {text}
              </p>
            )}
