...
export function getData(slug, locale, apiID, kind, preview) {
  ...
  if (kind == 'collectionType') {
    ...
    const apiUrl = `/${pluralize(
      apiID
    )}?filters[slug][$eq]=${slug}&locale=${locale}${previewParams}&populate[blocks][populate]=members.picture,header,buttons.link,items.button,items.header,items.image,items.tag,faq,blackType,whiteType,featuresCheck,button,cards,pricingCards.perks,articles,restaurants,author.picture,images,cards.image,cards.link,image&populate=localizations&populate[seo][populate]=metaSocial.image`;
